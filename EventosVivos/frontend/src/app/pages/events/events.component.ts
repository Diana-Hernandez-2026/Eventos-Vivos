...
export class EventsComponent implements OnInit {
  ...
  loadEvents(append = false) {
    ...
    this.api.getEvents(f).subscribe({
      ...
      error: (e) => { this.error = e.message || 'Error al cargar eventos'; this.loading = false; }
    });
  }

  loadMore() { this.loadEvents(true); }
  clearFilters() { this.filters = { titleSearch: '', type: '', status: '', venueId: '', startFrom: '', startTo: '' }; this.loadEvents(); }
  ...
  createEvent() {
    ...
    this.api.createEvent(payload).subscribe({
      next: (r) => {
        this.createSuccess = `Evento "${r.title}" creado exitosamente.`;
        this.creating = false;
        this.showCreate = false;
        this.loadEvents();
      },
      error: (e) => { this.createError = e.error?.detail || JSON.stringify(e.error?.extensions?.errors || e.message); this.creating = false; }
    });
  }

  viewReport(event: EventDto) {
    this.api.getOccupancyReport(event.id).subscribe({ next: r => this.selectedReport = r, error: e => alert(e.error?.detail || 'Error') });
  }

  openReservation(event: EventDto) {
    this.selectedEvent = event;
    this.reservation = { buyerName: '', buyerEmail: '', quantity: 1 };
    this.reservationError = ''; this.reservationSuccess = ''; this.paymentResult = null;
  }
  ...
  submitReservation() {
    ...
    this.api.createReservation({ eventId: this.selectedEvent.id, ...this.reservation }).subscribe({
      next: (r) => {
        ...
        this.reservationSuccess = 'Reserva creada. Estado: PendientePago';
        this.reserving = false;
      },
      error: (e) => { this.reservationError = e.error?.detail || JSON.stringify(e.error?.extensions?.errors || e.message); this.reserving = false; }
    });
  }
  ...
  confirmPayment() {
    ...
    this.api.confirmReservation(this.lastReservationId).subscribe({
      ...
      error: (e) => { this.reservationError = e.error?.detail || 'Error al confirmar pago'; this.confirming = false; }
    });
  }
}
